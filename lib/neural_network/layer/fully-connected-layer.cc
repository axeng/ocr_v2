...
namespace neural_network::layer
{
    FullyConnectedLayer::FullyConnectedLayer(
        ...
        , weights_(previous_layer.get_size(), neuron_count, -1, 1)
        , biases_(1, neuron_count, 0)
    {}

    FullyConnectedLayer::matrix_t
    FullyConnectedLayer::forward_propagation(const matrix_t& previous_values)
    {
        auto values = previous_values * this->weights_;
        values += this->biases_;
        values.apply([&](const matrix_t::data_t& element) {
            return this->activation_function_.activate(element);
        });
